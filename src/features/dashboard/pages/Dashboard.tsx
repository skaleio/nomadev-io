...
import { readDropiActiveImportId } from '@/features/dropi/lib/dropiSessionPrefs';
...
export default function Dashboard() {
  ...
  const syncDropi = useCallback(async () => {
    ...
    try {
      const activeImportId = readDropiActiveImportId(user.id);
      // Sin import activo: dashboard vacío hasta que el usuario suba o cargue una guía desde el historial.
      if (!activeImportId) {
        setDropiOrders([]);
        setMetaSpend(0);
