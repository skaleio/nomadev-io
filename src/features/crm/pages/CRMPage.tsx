...
// Mapeo bidireccional UI stage <-> DB lead.status
...
const dbLeadToLead = (l: DbLead): Lead & { stage: string } => ({
  id: l.id,
  name: l.name ?? '',
  email: l.email ?? '',
  phone: l.phone ?? '',
  company: l.company ?? '',
  value: Number(l.estimated_value ?? 0) || 0,
  source: l.source ?? '',
  lastContact: l.last_contact_at ?? l.updated_at ?? l.created_at ?? new Date().toISOString(),
  notes: l.notes ?? '',
  stage: STATUS_TO_STAGE[l.status ?? 'new'] ?? 'nuevo',
});
...
const CRMPage: React.FC = () => {
  ...
  const [editForm, setEditForm] = useState<Partial<Lead> & { stage?: string }>({});
  ...
  const loadCRMData = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('leads')
        .select('id,name,email,phone,company,status,estimated_value,source,notes,last_contact_at,updated_at,created_at')
        .eq('user_id', user.id)
      ...
      const mapped = (data ?? []).map(dbLeadToLead);
      setPipelineStages((prev) =>
        prev.map((stage) => {
          const stageLeads = mapped.filter((l) => l.stage === stage.id);
          return {
            ...stage,
            leads: stageLeads,
            totalValue: stageLeads.reduce((sum, l) => sum + (Number(l.value) || 0), 0),
          };
        })
      );
    } catch (err) {
  ...
  const handleAddLead = async (leadData: Partial<Lead> & { stage?: string }) => {
    if (!user?.id) {
    ...
    const status = STAGE_TO_STATUS[leadData.stage ?? 'nuevo'] ?? 'new';
    const { error } = await supabase.from('leads').insert({
      ...
      status,
      estimated_value: leadData.value ?? null,
      source: leadData.source ?? null,
      notes: leadData.notes ?? null,
      last_contact_at: new Date().toISOString(),
    });
    if (error) {
      toast.error(`No se pudo crear el lead: ${error.message}`);
      return;
    }
  ...
  const handleUpdateLead = async (leadId: string, updates: Partial<Lead> & { stage?: string }) => {
    if (!user?.id) return;
    ...
    if (updates.value !== undefined) patch.estimated_value = updates.value;
    if (updates.source !== undefined) patch.source = updates.source;
    if (updates.notes !== undefined) patch.notes = updates.notes;
    if (updates.stage !== undefined) patch.status = STAGE_TO_STATUS[updates.stage] ?? 'new';
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {loadError && (
          <div className="rounded-md border border-destructive/40 bg-destructive/10 px-4 py-3 text-destructive text-sm flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              <AlertCircle className="w-4 h-4 flex-shrink-0" />
              <span>No pudimos cargar tus leads: {loadError}</span>
            </div>
