...
import { resetDropiImportedDataForUser } from '@/features/dropi/lib/resetDropiImportedData';
...
function avatarFromSupabaseUser(supabaseUser: SupabaseUser): string | undefined {
...
function onboardingCompletedAtFromSupabaseUser(supabaseUser: SupabaseUser): string | undefined {
  const raw = supabaseUser.user_metadata?.onboarding_completed_at;
...
const emailRedirectUrl = (): string =>
  typeof window !== 'undefined' ? `${window.location.origin}/auth/success` : '';
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  // IMPORTANTE: isLoading siempre empieza en true para evitar que ProtectedRoute
  // renderice con una sesión cacheada inválida/expirada antes de verificar con Supabase.
  ...
  useEffect(() => {
            ...
            setUser((prev) => {
              ...
              const next = prev
                ? {
                    ...
                    avatarUrl: avatarFromSupabaseUser(su) ?? prev.avatarUrl,
                  }
                : buildQuickUser(su);
              saveCachedUser(next);
              return next;
    ...
    // Failsafe: si en 3s no llegó INITIAL_SESSION, salir del loading y descartar cache no verificada.
    failsafe = setTimeout(() => {
      if (isMounted) {
        commitUser(null);
        setIsLoading(false);
  ...
  const logout = useCallback(async () => {
    ...
    commitUser(null);
    if (sessionUserId) {
      clearDropiSessionPrefs(sessionUserId);
      clearDropiActiveImportId(sessionUserId);
    }
    try { localStorage.removeItem('dropi:lastImportAt'); } catch { /* noop */ }
