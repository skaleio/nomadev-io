...
/** Notifica al usuario una sola vez cuando su token Dropi expira y dispara un evento global. */
let tokenExpiredNotified = false;
const notifyDropiTokenExpired = () => {
  if (tokenExpiredNotified) return;
  tokenExpiredNotified = true;

  toast.error("Tu sesión de Dropi expiró", {
    description: "Reconectá tu cuenta para seguir viendo pedidos en tiempo real.",
    duration: 10000,
    action: {
      label: "Reconectar",
      onClick: () => {
        if (typeof window !== "undefined") window.location.assign("/dropi/connect");
      },
    },
  });

  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent("dropi:token-expired"));
  }
  // Permitir un nuevo aviso después de 30s para casos donde el usuario ignore.
  setTimeout(() => {
